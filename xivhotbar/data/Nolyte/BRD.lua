xivhotbar_keybinds_job['Base'] = {
 -- Hotbar #1
	{'b 1 5', 'ma',  'Dragonfoe Mambo', 'stpc', 'Eva 2','tile006(2)'},
 	{'b 1 6', 'ma',  'Sheepfoe Mambo', 'stpc', 'Eva 1','tile006(2)'},
 	{'b 1 7', 'ja',  'Marcato', 'me', 'Marcto',"tile528"},
 	{'b 1 8', 'ma',  "Army's Paeon", 'stpc', 'Dupe','tile078'},	
 	{'b 1 9', 'ma',  "Army's Paeon II", 'stpc', 'Dupe','tile078'},	
 	{'b 1 10', 'ma',  "Goddess's Hymnus", 'stpc', 'RR','tile800'},	

 -- Hotbar #2
	{'b 2 1', 'ma',  'Blade Madrigal', 'stpc', 'Acc 2','tile008'},
	{'b 2 2', 'ma',  'Sword Madrigal', 'stpc', 'Acc 1','tile008'},
	{'b 2 3', 'ma',  'Mage\'s Ballad III', 'stpc', 'MP 3','tile617'},
	{'b 2 4', 'ma',  'Mage\'s Ballad II', 'stpc', 'MP 2','tile617'},
	{'b 2 5', 'ma',  'Army\'s Paeon VI', 'stpc', 'HP',"tile638"},
	{'b 2 6', 'ma',  'Sentinel\'s Scherzo', 'stpc', 'Srzo','tile067'},
	{'b 2 10', 'ma',  'Chocobo Mazurka', 'me', 'Run','Overwhelming_Celerity'},
	{'b 2 7', 'ja',  'Troubadour', 'me', 'Troub.','tile761'},
	{'b 2 8', 'ja',  'Nightingale', 'me', 'Night.','tile856'},
	{'b 2 9', 'ja',  'Pianissimo', 'me', 'Pian.','tile753'},
 -- Hotbar #3
	{'b 3 1', 'ma',  'Magic Finale', 'stnpc', 'Dspl','tile877'},
	{'b 3 2', 'ma',  'Carnage Elegy', 'stnpc', 'Slow','tile075'},
	{'b 3 3', 'ma',  'Foe Requiem VII', 'stnpc', 'Req.',"tile700"},
	{'b 3 4', 'ma',  'Pining Nocturne', 'stnpc', '-M.Acc',"tile170"},
 	{'b 3 5', 'ma',  'Archer\'s Prelude', 'stpc', 'RAcc 2', "tile010"},
 	{'b 3 6', 'ma',  'Hunter\'s Prelude', 'stpc', 'RAcc 1', "tile010"},
	{'b 3 7', 'ja',  'Tenuto', 'me', 'Tenuto',"Generic_Feature_Icon"},
	{'b 3 8', 'ma',  'Mage\'s Ballad', 'stpc', 'MP 1',"tile617",},
	{'b 3 9', 'ma',  'Valor Minuet III', 'stpc', 'ATK III','tile595'},
	{'b 3 10', 'ma',  'Herculean Etude', 'stpc', 'STR+','tile414'},
 -- Hotbar #4
	{'b 4 2', 'ma',  'Valor Minuet V', 'stpc', 'ATK V','tile595'},
	{'b 4 1', 'ma',  'Valor Minuet IV', 'stpc', 'ATK IV','tile595'},
	{'b 4 3', 'ma',  'Honor March', 'stpc', 'Honor','Tome_of_Justice'},
	{'b 4 4', 'ma',  'Victory March', 'stpc', 'HST 2','_Advance!_'},

 	{'b 4 5', 'ma',  'Knight\'s Minne V', 'stpc', 'Min V','tile048'},	
 	{'b 4 6', 'ma',  'Knight\'s Minne IV', 'stpc', 'Min IV','tile048'},	
	{'b 4 7', 'ma',  'Horde Lullaby', 'stnpc', 'Slpga','tile448'},	
	{'b 4 8', 'ma',  'Horde Lullaby II', 'stnpc', 'SlpgaII','tile250(2)'},
	{'b 4 9', 'ma',  'Foe Lullaby II', 'stnpc', 'SlpII','tile944'},		
	{'b 4 10', 'ma',  "Maiden's Virelai", 'stnpc', 'Charm','Evelynn_Allure_HD'},		
	-- {'f 2 1', 'ma',  'Fire Threnody', 'stnpc', 'Fire-'},
	-- {'f 2 2', 'ma',  'Ice Threnody', 'stnpc', 'Ice-'},
	-- {'f 2 3', 'ma',  'Wind Threnody', 'stnpc', 'Wind-'},
	-- {'f 2 4', 'ma',  'Earth Threnody', 'stnpc', 'Earth-'},
	-- {'f 2 5', 'ma',  'Light Threnody', 'stnpc', 'Light-'},
	-- {'f 2 6', 'ma',  'Dark Threnody', 'stnpc', 'Dark-'},
	--{'f 1 9', 'ma',  'Horde Lullaby', 'stnpc', 'Slepga'},
	--{'f 1 10', 'ma',  'Horde Lullaby II', 'stnpc', 'SlepgaII'},

	--{'f 2 9', 'ma',  'Foe Lullaby', 'stnpc', 'Sleep'},
	--{'f 2 10', 'ma',  'Foe Lullaby II', 'stnpc', 'SleepII'},
	
	{'f 3 9', 'ja',  'Clarion Call', 'me', 'C.Call','tile1003'},
	{'f 3 10', 'ja',  'Soul Voice', 'me', 'S.Voice','tile684'},

	{'b 6 1', 'ma',  'Light Carol II', 'stpc', 'Light',"BarLight"},
	{'b 6 2', 'ma',  'Dark Carol II', 'stpc', 'Dark',"BarDarkness"},
	{'b 6 3', 'ma',  'Ice Carol II', 'stpc', 'Ice',"BarParalyze"},
	{'b 6 4', 'ma',  'Wind Carol II', 'stpc', 'Wind',"BarSilence"},
	{'b 6 5', 'ma',  'Lightning Carol II', 'stpc', 'Thun.',"BarThunder"},	
	
	{'b 6 7', 'ma',  'Light Threnody II', 'stnpc', 'Light',"Light_Threnody"},
	{'b 6 8', 'ma',  'Dark Threnody II', 'stnpc', 'Dark',"Dark_Threnody"},	
	{'b 6 9', 'ma',  'Ice Threnody II', 'stnpc', 'Ice',"Ice_Threnody"},	
	{'b 6 10', 'ma',  'Fire Threnody II', 'stnpc', 'Fire',"Fire_Threnody"},	
		
}

xivhotbar_keybinds_job['Sword'] = {
	-- Battle
	{'b 1 1', 'weaponskill',  'Savage Blade', 'stnpc', 'Savge', 'Imperator'}, 
	{'b 1 2', 'weaponskill',  'Circle Blade', 't', 'AoE', 'Sword-AoE'}, 
	{'b 1 3', 'weaponskill',  'Sanguine Blade', 't', 'Sang.', 'Scarlet'}, 
	
	{'f 1 1', 'weaponskill',  'Savage Blade', 'stnpc', 'Savge', 'Imperator'}, 
	{'f 1 2', 'weaponskill',  'Circle Blade', 't', 'AoE', 'Sword-AoE'}, 
	--{'f 1 3', 'weaponskill',  'Sanguine Blade', 't', 'Sang.', 'Scarlet'}, 
}
xivhotbar_keybinds_job['Dagger'] = {
	-- Battle
	{'b 1 1', 'weaponskill', 'Rudra\'s Storm', 'stnpc', 'Rudra', 'Zesho'},
    {'b 1 3', 'weaponskill', 'Evisceration', 'stnpc', 'Evisc.', 'Dagger-Multi'},
    {'b 1 2', 'weaponskill', 'Mordant Rime', 'stnpc', 'Mordnt', 'Dagger-Single2'},
	{'b 1 4', 'weaponskill', 'Aeolian Edge', 't', 'Aeolian.', 'Dagger-AoE'},
	
	{'f 1 1', 'weaponskill', 'Rudra\'s Storm', 'stnpc', 'Rudra', 'Zesho'},
    {'f 1 3', 'weaponskill', 'Evisceration', 'stnpc', 'Evisc.', 'Dagger-Multi'},
    {'f 1 2', 'weaponskill', 'Mordant Rime', 'stnpc', 'Mordnt', 'Dagger-Single2'},
	{'f 1 4', 'weaponskill', 'Aeolian Edge', 't', 'Aeolian.', 'Dagger-AoE'},
}

xivhotbar_keybinds_job['PLD'] = {
    {'f 1 7',  'ma', 'Cure IV', 'stpc', 'CureIV'},
    {'f 1 8',  'ma', 'Banishga', 't', 'Bnshga', 'Angel'},
    {'f 1 9',  'ma', 'Flash', 'stnpc', 'Flash', 'Flash'},
    {'f 2 7',  'ja', 'Sentinel', 'me', 'Sntnl', 'Sentinel'},
    {'f 2 8',  'ja', 'Cover', 'stpc', 'Cver', 'Cover'},
	{'f 1 5', 'ja', 'Holy Circle', 'me', 'H.Crcl', 'HolyCircle'}
}

xivhotbar_keybinds_job['DNC'] = {
	-- Battle
    {'f 2 2', 'ja', 'Haste Samba', 'me', 'Haste', 'HasteSamba'},
	{'f 2 10', 'ja', 'Chocobo Jig', 'me', 'Choco', 'Overwhelming_Celerity'},
	
    {'f 3 1', 'ja', 'Curing Waltz III', 'stpc', 'CureIII', 'WaltzIII'},
	{'f 3 2', 'ja', 'Divine Waltz', 'stpc', 'Cura', 'Divine'},
	{'f 3 3', 'ja', 'Healing Waltz', 'stpc', 'Esuna', 'HealingWaltz'},
	{'f 4 7', 'ja', 'Reverse Flourish', 'me', 'Rvrse', 'Reverse'},
	{'f 4 8', 'ja', 'Building Flourish', 'me', 'WS+', 'BuildingFlourish'},

	{'f 4 1', 'ja', 'Box Step', 't', 'Box', 'Qiyana_Terrashape_Rock_HD'},	
	{'f 4 2', 'ja', 'Stutter Step', 't', 'MEVA-', "Qiyana_Terrashape_Brush_HD"},
	{'f 4 3', 'ja', 'Quickstep', 't', 'EVA-', "Qiyana_Terrashape_HD"},
	{'f 4 9', 'ja', 'Spectral Jig', 'me', 'Spctrl', 'Smoke_Screen'},	
	{'f 4 10', 'ja', 'Contradance', 'me', 'Heal+', "CuringWaltz"},
}

xivhotbar_keybinds_job['NIN'] = {
	-- Battle
	{'f 4 1', 'ma', 'Utsusemi: Ichi', 'me', 'Utsu', 'UtsuIchi'},
	{'f 4 2', 'ma', 'Utsusemi: Ni', 'me', 'Utsu', 'UtsuNi'},
	{'f 4 9', 'ma', 'Monomi: Ichi', 'me', 'Sneak'},
	{'f 4 10', 'ma', 'Tonko: Ichi', 'me', 'Invis'},
}

xivhotbar_keybinds_job['WHM'] = {
	{'f 1 7', 'ma', 'Raise II', 'stpc', 'RsII',"tile844"},
	{'f 1 5', 'ma', 'Dia II', 'stnpc', 'Dia',"tile455"},	
 	{'f 1 6', 'ma', 'Cure III', 'stpc', 'CureIII',"tile078"},
	{'f 3 2', 'ma', 'Paralyna', 'stpc', 'Para',"Elixir_U"},
	{'f 3 3', 'ma', 'Silena', 'stpc', 'Slna',"Frigid_Wurm_Goo"},
	{'f 3 4', 'ma', 'Stona', 'stpc', 'Stna',"Super_Elixir"},
	{'f 3 5', 'ma', 'Viruna', 'stpc', 'Virna',"Elixir_S"},
	{'f 3 6', 'ma', 'Cursna', 'stpc', 'Crsna',"Completely_Legal_Performance-Enhancing_Serum"},
	{'f 3 7', 'ma', 'Poisona', 'stpc', 'Psnna',"Elixir_B"},

	{'f 3 8', 'ma', 'Auspice', 'me', 'Auspice',"Chapter_2-_Igniting_Burst"},
	{'f 1 8', 'ma', 'Erase', 'stpc', 'Erase',"Epilogue-_Eternal_Oasis"},
 	{'f 3 1', 'ma', 'Cure IV', 'stpc', 'CureIV',"tile792"},
	{'f 4 1', 'ma', 'Curaga III', 'stpc', 'CuraIII',"Searing_Light"},	
	{'f 4 2', 'ma', 'Barblizzara', 'me', 'Blzra',"tile053"},	
	{'f 4 3', 'ma', 'Barfira', 'me', 'Fira',"tile054"},	
	{'f 4 4', 'ma', 'Barstonra', 'me', 'Stnra',"tile031"},	
	{'f 4 5', 'ma', 'Baraera', 'me', 'Aera',"tile052"},	
	{'f 4 6', 'ma', 'Barthundra', 'me', 'Thndra',"tile055"},	
	{'f 4 8', 'ma', 'Reraise II', 'me', 'ReraiseII',"tile800"},
	{'f 4 10', 'ma', 'Cura', 'me', 'Cura',"tile025"},	
	{'f 2 7', 'ja', 'Divine Seal', 'me', 'D.Seal',"Symbol_of_Faith"},	
	{'f 2 8', 'ma', 'Haste', 'stpc', 'Haste',"_Advance!_"},	
	{'f 2 1', 'ma', 'Banish II', 'stnpc', 'Bnsh',"Heaven's_Palm"},		
	{'f 2 2', 'ma', 'Banishga II', 'stnpc', 'Bnshga',"Fire_Jurisdiction"},	
	{'f 2 3', 'ma', 'Silence', 'stnpc', 'Slnce',"tile277"},		
	{'f 2 4', 'ma', 'Stoneskin', 'me', 'StnSkn',"tile639"},
	{'f 2 5', 'ma', 'Aquaveil', 'me', 'Veil',"tile630"},		
	{'f 2 6', 'ma', 'Blink', 'me', 'Blnk',"tile191"},				

	{'f 1 10',  'input', '//exec Balladx3', 'me', 'Ballad','tile617'},		
}
xivhotbar_keybinds_job['BLM'] = {
	{'b 4 10', 'ja', 'Elemental Seal', 'me', 'E.Seal'},
	{'f 4 10', 'ja', 'Elemental Seal', 'me', 'E.Seal'},
	{'f 4 8', 'ma', 'Warp', 'me', 'Warp'},
 	{'f 4 9', 'ma', 'Warp II', 'stpc', 'WarpII'},
 	{'f 2 10', 'ma', 'Escape', 'me', 'Esc'},
 	{'f 4 4', 'ma', 'Sleepga', 't', 'Slpga'},
 	{'f 4 1', 'ma', 'Stone III', 't', 'Stone'},
 	{'f 4 2', 'ma', 'Stonega II', 't', 'Stnga'},
	{'f 2 9', 'ma', 'Ice Spikes', 'me', 'Ice'},
	{'f 2 1', 'ma', 'Poisonga', 't', 'Psnga', "Spider_Venom"},
	{'f 2 3', 'ma', 'Poison II', 't', 'Psn'},	
	{'f 3 2', 'ma', 'Choke', 't', 'Choke'},	
	{'f 3 4', 'ma', 'Bio II', 't', 'Bio'},	
	{'f 3 3', 'ma', 'Burn', 't', 'Burn'},	
	{'f 3 1', 'ma', 'Shock', 't', 'Shock'},	
	{'f 1 10', 'ma', 'Stun', 't', 'Stun',"tile959"},		
}
xivhotbar_keybinds_job['RDM'] = {
	--{'b 1 9', 'ma', 'Phalanx', 'me', 'Phalanx', 'Stoneskin'},
 	{'b 4 1', 'ma', 'Cure III', 'stpc', 'CureIII'},
	{'b 4 2', 'ma', 'Raise II', 'stpc', 'Raise'},
	{'b 4 3', 'ma', 'Haste II', 'stpc', 'Haste'},
	{'b 4 4', 'ma', 'Flurry II', 'stpc', 'Flurry'},
	{'b 4 5', 'ma', 'Protect V', 'stpc', 'Prtct'},
	{'b 4 6', 'ma', 'Shell V', 'stpc', 'Shell'},
	
	{'f 1 10', 'ja', 'Convert', 'me', 'Cnvrt'},
	{'f 3 3', 'ma', 'Refresh', 'stpc', 'Rfresh'},
	{'f 2 4', 'ma', 'Haste', 'stpc', 'Haste'},
	{'f 2 1', 'ma', 'Dia II', 't', 'Dia'},
	{'f 2 7', 'ma', 'Stoneskin', 'me', 'StnSkn',"tile639"},
	{'f 2 8', 'ma', 'Enfire', 'me', 'En-Ele'},
	{'f 1 9', 'ma', 'Phalanx', 'me', 'Phalnx'},

	{'f 3 1', 'ma', 'Protect III', 'me', 'Prtct'},
	{'f 3 2', 'ma', 'Shell II', 'me', 'Shell'},	
	{'f 4 9', 'ma', 'Sneak', 'me', 'Sneak'},
	{'f 4 10', 'ma', 'Invisible', 'me', 'Invis'},
}
return xivhotbar_keybinds_job
